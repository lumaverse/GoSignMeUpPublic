function InstructorIdentityWidget(dashboard) {
    var self = this;

    self.State.dashboard = dashboard;
}

InstructorIdentityWidget.prototype = new WidgetBase();

InstructorIdentityWidget.constructor = InstructorIdentityWidget;

InstructorIdentityWidget.prototype.Options = {
};

InstructorIdentityWidget.prototype.State = {
    container: null,
    dashboard: null,
    widget: null
};

InstructorIdentityWidget.prototype.RenderImplementation = function () {
    var self = this;
    ...
    var getProfileImage = function () {
        ...
        if (supervisorData.PhotoImage != null && supervisorData.PhotoImage != '') {
            profileImage = self.State.dashboard.Options.profileImagePath + supervisorData.PhotoImage + '?rnd=' + Math.random();
        }
    };
    getProfileImage();
    ...
    self.State.widget = Ext.create('Ext.form.Panel', self.getWidgetDefaults({
        ...
        frame: true,
        title: 'Identity',
        url: config.getUrl('public/instructor/saveidentity'),
